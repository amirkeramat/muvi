...
const Card = ({ item, type, shape }) => {
  return (
    <div className="md:hover:opacity-90  cursor-pointer p-4 md:p-0 relative">
      {shape === "ver" ? (
        <LazyLoadImage
          placeholder={
            <p className="w-[200px] max-w-[400px] h-[400px] flex justify-center items-center bg-gray-800 ">
              <img src="./title-logo.png" alt="" />
            </p>
          }
          width={"100%"}
          src={`${POSTER_W780}/${
            item?.posterPath ? item?.posterPath : item.profilePath
          }`}
